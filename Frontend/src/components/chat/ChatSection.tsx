...
export const ChatSection = ({
  ...
  useEffect(() => {
    if (conversationQuery.data && !conversationQuery.isLoading) {
      if (conversationQuery.data.conversation_id) {
        setConversationId(conversationQuery.data.conversation_id);
      }
      const history: Message[] = conversationQuery.data.messages.map((m: any) => ({
        id: m.message_id || Date.now().toString(),
        text: m.content || m.response || "",
  ...
  const isLoading = typeof isLoadingProp === "boolean" ? isLoadingProp : (chatMutation.isPending || conversationQuery.isLoading);
