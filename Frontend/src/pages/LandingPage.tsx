...
const LandingPage = () => {
    ...
    <div className="min-h-screen bg-black font-sans relative overflow-x-hidden text-white">
              ...
              user ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className="rounded-full h-9 w-9 bg-white/5 border border-white/10 hover:bg-white/10 ml-4">
                      {user.photoURL ? (
                ...
                <Button 
                  ...
                  variant="outline" 
                  className="ml-4 bg-white/5 border-white/10 hover:bg-white/10 text-white"
                >
                  Login / Signup
                </Button>
              )
      ...
      <div className={`mobile-menu ${isMobileMenuOpen ? 'open' : ''}`}>
        ...
        {!loading && (
          user ? (
            <>
              <div className="mobile-nav-item" onClick={() => { setIsMobileMenuOpen(false); navigate("/dashboard"); }}>Dashboard</div>
              <div className="mobile-nav-item text-red-400" onClick={() => { setIsMobileMenuOpen(false); handleLogout(); }}>Log out</div>
            </>
          ) : (
            <div className="mobile-nav-item text-blue-400" onClick={() => { setIsMobileMenuOpen(false); navigate("/auth"); }}>Login / Signup</div>
          )
        )}
      </div>
  ...
  <div className="container">
    ...
    <div style={{ marginTop: 24 }}>
      <button className="liquid-btn" onClick={() => navigate(user ? "/dashboard" : "/auth")} aria-label="Try Now">
