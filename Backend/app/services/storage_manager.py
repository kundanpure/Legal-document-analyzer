...
class StorageManager:
    ...
    async def _save_audio_gcs(self, audio_data: bytes, filename: str, voice_id: str = None) -> str:
        ...
        # Make publicly accessible (might fail on uniform bucket-level access)
        try:
            await asyncio.to_thread(blob.make_public)
        except Exception as e:
            self.logger.warning(f"Could not set public ACL (Uniform Bucket-Level Access enabled?): {e}")
